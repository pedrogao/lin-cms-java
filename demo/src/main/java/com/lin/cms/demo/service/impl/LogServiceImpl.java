...
public class LogServiceImpl extends AbstractService<Log> implements LogService {
    ...
    public PageResult getLogs(Integer page, Integer count, String name, Date start, Date end) {
        ...
        List<Log> logs = logMapper.findLogsByUsernameAndRange(name, start, end);
    ...
    public PageResult searchLogs(Integer page, Integer count, String name, String keyword, Date start, Date end) {
        PageHelper.startPage(page + 1, count);
        List<Log> logs = logMapper.searchLogsByUsernameAndKeywordAndRange(name, keyword, start, end);
